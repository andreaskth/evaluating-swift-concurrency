...
class NQueens: Benchmark {
    ...
    private func runAsync(numberOfQueens: Int, numberOfIterations: Int, iterationsToDiscard: Int) async {
        ...
        for i in 1...numberOfIterations {
            ...
            let time = await self.asyncQueens(n: numberOfQueens)
        ...
        print("\n(async, average, \(numberOfQueens) queens) Done in an average of \(asyncAvgTime) seconds.\n")
    }
    ...
    private func runGCD(numberOfQueens: Int, numberOfIterations: Int, iterationsToDiscard: Int) {
        ...
        for i in 1...numberOfIterations {
            ...
            let time = GCDQueens(n: numberOfQueens)
        ...
        print("\n(GCD, average, \(numberOfQueens) queens) Done in an average of \(GCDAvgTime) seconds.\n")
    }

    private func asyncQueens(n: Int) async -> TimeInterval {

        let before = ProcessInfo.processInfo.systemUptime
        let res = await solutionsAsync(n)
    ...
    private func GCDQueens(n: Int) -> TimeInterval {

        let before = ProcessInfo.processInfo.systemUptime
        let res = solutionsGCD(n)
        let after = ProcessInfo.processInfo.systemUptime
        let time = after - before
