...
class MatrixMultiplication: Benchmark {
    ...
    private func runAsync(matrixRows: Int, matrixColumns: Int, numberOfIterations: Int, iterationsToDiscard: Int) async {
        ...
        for i in 1...numberOfIterations {
            ...
            let (time, _) = await self.asyncMatrixMult(leftMatrix: A, rightMatrix: B)
        ...
        print("\n(async, average, (\(matrixRows)x\(matrixColumns)) matrix) Done in an average of \(asyncAvgTime) seconds.\n")
    }
    ...
    private func runGCD(matrixRows: Int, matrixColumns: Int, numberOfIterations: Int, iterationsToDiscard: Int) {
        ...
        for i in 1...numberOfIterations {
            ...
            let (time, _) = GCDMatrixMult(leftMatrix: A, rightMatrix: B)
        ...
        print("\n(GCD, average, (\(matrixRows)x\(matrixColumns)) matrix) Done in an average of \(GCDAvgTime) seconds.\n")
    }
    ...
    private func asyncMatrixMult(leftMatrix: [[Int]], rightMatrix: [[Int]]) async -> (TimeInterval, [[Int]]) {
        ...
        return (time, await res.get())
    }

    private func GCDMatrixMult(leftMatrix: [[Int]], rightMatrix: [[Int]]) -> (TimeInterval, [[Int]]) {
    ...
    private func verifyMult() {
        ...
        let (_, actualGCD) = GCDMatrixMult(leftMatrix: leftMatrix, rightMatrix: rightMatrix)
        precondition(expected == actualGCD, "GCD matrix multiplication went wrong")

        Task {
            let (_, actualAsync) = await asyncMatrixMult(leftMatrix: leftMatrix, rightMatrix: rightMatrix)
